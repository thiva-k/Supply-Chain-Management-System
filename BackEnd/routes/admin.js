...
router.get('/neworder', (req, res) => {
    const sql = "select * from(((train left outer join places on(train.destination = places.district)) left outer join customer_order using (order_id)) left outer join hold using(order_id)) where((order_id is not null) and state = 'created') order by date_time desc";
...
router.get('/scheduledorder', (req, res) => {
    const sql = "select * from(((train left outer join places on(train.destination = places.district)) left outer join customer_order using (order_id)) left outer join hold using(order_id)) where((order_id is not null) and state = 'trainsheduled' ) order by date_time desc";
    db.query(sql, (err, result) => {
        res.send(result);
    })
});
...
router.get('/scheduletrainto', (req, res) => {
    ...
    const sql = "select * from (places right outer join hold using(order_id)) left outer join customer_order using(order_id) left outer join train on (train.destination = places.district ) where(district=? and state = 'created')  group by order_id; ";

...
router.get('/setroutes', (req, res) => {
    ...
    const sql = ["select * from (customer_order natural join places) join view_store_without_auth on (view_store_without_auth.first_name = district)where (store_id=?  and state='recievedstore')", "select * from route natural join leads where store_id = ?"]
...
router.get('/finishedorders', (req, res) => {
    const data = req.query[0];
    // console.log(data)
    const sql = "select * from (customer_order natural join places) left outer join view_store_without_auth on(places.district=view_store_without_auth.first_name) where store_id=" + db.escape(data) + " and state='delivered' order by date_time desc;"
...
router.get('/neworder', (req, res) => {
    const data = req.query[0];
    // console.log(data)
    const sql = "select * from (customer_order natural join places) left outer join view_store_without_auth on(places.district=view_store_without_auth.first_name) where store_id=" + db.escape(data) + " and state='created' order by date_time desc;"
    db.query(sql, [data, data], (err, result) => {
        res.send(result);
        // console.log(err)
    })
...
router.post('/setroutes', (req, res) => {   
    ...
    const sql = data.map(d => (
        "Insert into transports values(" + db.escape(d.route_id) + "," + db.escape(d.order_id) + ")"
    ))
...
router.get('/scheduletruck', (req, res) => {
    ...
    const sql = [
        "select * from route where store_id=" + db.escape(store_id) + "",

        "select * from ((leads left outer join route using(route_id)) left outer join transports using(route_id)) left outer join customer_order using(order_id) where (state='routescheduled' and store_id=" + db.escape(store_id) + ")",
...
router.get('/detail', (req, res) => {
    const data = req.query['0'];
    // console.log(data);
    const sql = ["select count(route_id) as data from leads where store_id=?", "select count(truck_id) as data from truck where store_id=?", "select count(worker_id) as data from hires where store_id=?", "select count(*) as data from customer_order where order_id in (select order_id from places where (district = (select first_name from view_store_without_auth where store_id = ?)) and state='traintransport')"]
...
router.post('/addTruck', (req, res) => {
    ...
    // const id = getUniqId('tru')
    const sql = "INSERT INTO truck (truck_id,capacity, store_id) VALUES (?,?,?);"
    db.query(sql, [a.truck_number, a.capacity,a.store_id], (err, result) => { if (err)console.log(err); });
});

...
router.post('/', (req, res) => {
    db.getConnection(function (err, connection) {
        connection.beginTransaction(function (err) {
            ...
            const sql4 = "insert into customer_order(order_id,total_price,capacity,customer_id,address,city,district) values (?,?,?,?,?,?,?)";
            db.query(sql4, [order_id, cost, capacity,user.customer_id, a.address, a.city, a.district], (error, result) => {
                if (error) {
            ...
            a.carts.forEach(element => {
                ...
                const sql2 = "insert into hold(order_id,product_id,count) values(?,?,?)";
